...
export default function TransactionItem({ transaction, category, currencySymbol, onPress }: Props) {
  ...
  const styles = useMemo(() => StyleSheet.create({
    container: { flexDirection: 'row', alignItems: 'center', paddingHorizontal: SPACING.lg, paddingVertical: SPACING.sm, minHeight: 64, backgroundColor: C.bg },
    iconContainer: { width: 44, height: 44, borderRadius: RADIUS.md, alignItems: 'center', justifyContent: 'center', marginRight: SPACING.md },
    icon: { fontSize: 22 },
    info: { flex: 1 },
    label: { fontFamily: T.fonts.semibold, fontSize: T.sizes.md, color: C.text },
    subLabel: { fontFamily: T.fonts.body, fontSize: T.sizes.xs, color: C.text2, marginTop: 2 },
    amount: { fontFamily: 'SpaceMono-Regular', fontSize: T.sizes.md, textAlign: 'right', marginLeft: SPACING.sm },
  }), [C]);
